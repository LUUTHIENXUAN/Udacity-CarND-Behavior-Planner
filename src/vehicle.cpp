    ...
    if (get_vehicle_ahead(predictions, lane, vehicle_ahead)) {
        ...
        if (get_vehicle_behind(predictions, lane, vehicle_behind)) {
            ...
            if (this->s + this->preferred_buffer < vehicle_ahead.s){
              ...
              new_velocity = min(max_velocity_accel_limit,
                                 this->target_speed);
            }
            else new_velocity = vehicle_ahead.v;

        }
        else {

          float max_velocity_in_front = vehicle_ahead.s
                                        - this->s
                                        - this->preferred_buffer
                                        + vehicle_ahead.v
                                        - 0.5 * (this->a);

          new_velocity = min (min(max_velocity_in_front,
...
vector<Vehicle> Vehicle::constant_speed_trajectory() {
    ...
    */
    ...
    vector<Vehicle> trajectory = {Vehicle(this->lane,
                                          this->s,
                                          this->v,
                                          this->a,
                                          this->state),
...
vector<Vehicle> Vehicle::keep_lane_trajectory(map<int,
                                              vector<Vehicle>> predictions) {
    /*
    Generate a keep lane trajectory.
    */
    vector<Vehicle> trajectory = {Vehicle(lane,
                                          this->s,
                                          this->v,
                                          this->a,
                                          state)};
    ...
    trajectory.push_back(Vehicle(this->lane,
                                 new_s,
                                 new_v,
                                 new_a,
                                 "KL"));
...
vector<Vehicle> Vehicle::prep_lane_change_trajectory(string state, map<int,
                                                     vector<Vehicle>> predictions) {
...
vector<Vehicle> Vehicle::lane_change_trajectory(string state,
    ...
    */
    ...
    trajectory.push_back( Vehicle (new_lane,
                                   kinematics[0],
                                   kinematics[1],
                                   kinematics[2],
                                   state));
    ...
    */
         ...
         it != predictions.end(); ++it)
    {
        ...
        if (temp_vehicle.lane == this->lane
            && temp_vehicle.s < this->s && temp_vehicle.s > max_s)
        {
